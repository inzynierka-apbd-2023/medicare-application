...
export default function SchedulePage() {
  const data = {
    services: [
      { id: "service_general_consultation", name: "General Consultation", specializationId: "spec_general_medicine", doctorIds: ["doc_alice_heart", "doc_bob_vessel"] },
      { id: "service_blood_test", name: "Blood Test", specializationId: "spec_lab_services", doctorIds: ["doc_carol_serum"] },
      { id: "service_teleconsultation", name: "Teleconsultation", specializationId: "spec_general_medicine", doctorIds: ["doc_alice_heart"] },
      { id: "service_prescription_renewal", name: "Prescription Renewal", specializationId: "spec_pharmacy", doctorIds: ["doc_deborah_dose"] }
    ],
    specializations: [
      { id: "spec_general_medicine", name: "General Medicine", serviceIds: ["service_general_consultation", "service_teleconsultation"], doctorIds: ["doc_alice_heart", "doc_bob_vessel"] },
      { id: "spec_lab_services", name: "Lab Services", serviceIds: ["service_blood_test"], doctorIds: ["doc_carol_serum"] },
      { id: "spec_pharmacy", name: "Pharmacy", serviceIds: ["service_prescription_renewal"], doctorIds: ["doc_deborah_dose"] }
    ],
    doctors: [
    ...
    <div className="min-h-screen bg-gray-100 overflow-x-hidden">
      ...
      <div className="container mx-auto max-w-[160rem] px-4 sm:px-6 lg:px-8">
        <main className="pt-24 pb-10">
          <h1 className="text-3xl font-bold text-blue-700 mb-8">Schedule Appointment</h1>
            ...
            <div className="w-3/4 bg-white rounded-2xl shadow-md p-6">
              ...
              <div className="flex flex-col md:flex-row md:space-x-4 space-y-4 md:space-y-0 mb-6">
                <select
                  ...
                  onChange={e => { setSelectedService(e.target.value); setSelectedDoctor(""); }}
                  disabled={!!selectedDoctor}
                  className="flex-1 p-2 border rounded-lg"
                >
                  <option value="">Select Service</option>
                  {serviceOptions.map(id => <option key={id} value={id}>{getServiceName(id)}</option>)}
                </select>

                <select
                  value={selectedSpec}
                  onChange={e => { setSelectedSpec(e.target.value); setSelectedService(""); setSelectedDoctor(""); }}
                  disabled={!!selectedDoctor}
                  className="flex-1 p-2 border rounded-lg"
                >
                  <option value="">Select Specialization</option>
                  {specOptions.map(id => <option key={id} value={id}>{getSpecName(id)}</option>)}
                </select>
                ...
                >
                  ...
                  {doctorOptions.map(id => <option key={id} value={id}>{getDoctorName(id)}</option>)}
                </select>
              </div>
