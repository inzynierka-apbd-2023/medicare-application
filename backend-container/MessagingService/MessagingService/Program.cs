...
static (string ConnectionString, string Source, bool UseAzureDefaultCredential) NormalizeConnectionString(IConfiguration config)
{
    ...
    else if (!string.IsNullOrWhiteSpace(config["ConnectionStrings__MessagingDb"])) { cs = config["ConnectionStrings__MessagingDb"]; src = "ConnectionStrings__MessagingDb env var"; }
    else { cs = config.GetConnectionString("MessagingDb"); src = "appsettings"; }
