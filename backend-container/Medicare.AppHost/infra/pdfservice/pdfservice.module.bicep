...
param pdfservice_containerimage string

resource pdfservice 'Microsoft.App/containerApps@2024-03-01' = {
  name: 'pdfservice'
  location: location
  properties: {
    configuration: {
      ...
      ingress: {
        external: false
        targetPort: pdfservice_containerport
        transport: 'http'
      }
      registries: [
        {
    ...
    template: {
      containers: [
        {
          image: pdfservice_containerimage
          name: 'pdfservice'
          env: [
            {
            ...
            {
              name: 'HTTP_PORTS'
              value: pdfservice_containerport
            }
            {
