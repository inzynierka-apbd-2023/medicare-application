...
param labservice_containerport string
...
param labservice_containerimage string

resource labservice 'Microsoft.App/containerApps@2024-03-01' = {
  name: 'labservice'
  location: location
  properties: {
    configuration: {
      ...
      ingress: {
        external: false
        targetPort: labservice_containerport
        transport: 'http'
      }
      registries: [
        {
    ...
    template: {
      containers: [
        {
          image: labservice_containerimage
          name: 'labservice'
          env: [
            {
            ...
            {
              name: 'HTTP_PORTS'
              value: labservice_containerport
            }
            {
