...
param archiveservice_containerport string
...
param archiveservice_containerimage string

resource archiveservice 'Microsoft.App/containerApps@2024-03-01' = {
  name: 'archiveservice'
  location: location
  properties: {
    configuration: {
      ...
      ingress: {
        external: false
        targetPort: archiveservice_containerport
        transport: 'http'
      }
      registries: [
        {
    ...
    template: {
      containers: [
        {
          image: archiveservice_containerimage
          name: 'archiveservice'
          env: [
            {
            ...
            {
              name: 'HTTP_PORTS'
              value: archiveservice_containerport
            }
            {
