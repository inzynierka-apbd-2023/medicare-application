...
public class AdminController : ControllerBase
{
    ...
    [HttpGet("debug/patient/{patientId}")]
    [AllowAnonymous]
    public async Task<ActionResult<object>> GetPatientDebug(Guid patientId)
    {
        if (_env.IsProduction()) return Forbid();
        var sub = await _db.SubscriptionContracts
            .Include(s => s.DefaultPaymentMethod)
            .OrderByDescending(s => s.PeriodEnd)
            .FirstOrDefaultAsync(s => s.PatientId == patientId && s.Status == SubscriptionStatus.Active);
            
        var payments = await _db.AppointmentPayments
            .Where(ap => ap.PatientId == patientId)
            .OrderByDescending(ap => ap.CreatedAt)
            .ToListAsync();
            
        return Ok(new { Subscription = sub, Payments = payments });
    }
    [HttpPost("debug/process-appointment")]
    [AllowAnonymous]
    public async Task<IActionResult> DebugProcessAppointment([FromBody] ProcessAppointmentRequest req)
    {
         if (_env.IsProduction()) return Forbid();
         
         // 0. Check if already processed
         var existing = await _db.AppointmentPayments.FirstOrDefaultAsync(ap => ap.AppointmentId == req.AppointmentId);
         if (existing != null)
         {
             return Ok(new { Message = "Already processed", Payment = existing });
         }

         // Logic from Consumer
         var sub = await _db.SubscriptionContracts
             .OrderByDescending(s => s.PeriodEnd)
             .FirstOrDefaultAsync(s => s.PatientId == req.PatientId && s.Status == SubscriptionStatus.Active);
         ...
         var visitsUsed = await _db.AppointmentPayments
            .CountAsync(ap => ap.PatientId == req.PatientId && ap.CreatedAt >= startOfMonth);
            
         // Fix for MaxValue: Ensure we don't overflow or weird logic
         bool isFree = visitsUsed < plan.FreeVisitsPerMonth;
         long amount = isFree ? 0 : 30000;
         ...
         return Ok(new { Payment = payment, IsFree = isFree, Plan = planCode, VisitsUsed = visitsUsed, MaxVisits = plan.FreeVisitsPerMonth });
    }
}
