...
static (string ConnectionString, string Source, bool UseAzureDefaultCredential) NormalizeConnectionString(IConfiguration config)
{
    ...
    else if (!string.IsNullOrWhiteSpace(config["ConnectionStrings__PractitionerServiceDb"])) { cs = config["ConnectionStrings__PractitionerServiceDb"]; src = "ConnectionStrings__PractitionerServiceDb env var"; }
    else { cs = config.GetConnectionString("PractitionerServiceDb"); src = "appsettings"; }
